...
void dash_board(void)
{
    ...
    spd = read_adc(0x04) / 10.33;
    clcd_putch(spd / 10 + 48, (0xC0 + (14)));
    clcd_putch(spd % 10 + 48, (0xC0 + (15)));
...
void view_log(void)
{
    ...
    else
    {
        ...
        for (int i = 0; i < event_count; i++)
        {
            val = (shift_idx + i) % 10;
            store_add = val * 10;
            store_arr[i][0] = read_external_eeprom(store_add++);
        ...
        if (key == 2 && store_idx < event_count - 1)
        {
            store_idx++;
        } else if (key == 1 && store_idx > 0)
        {
            store_idx--;
        }

        clcd_putch(store_idx + 48, (0xC0 + (0)));
        clcd_putch(' ', (0xC0 + (1)));
        clcd_print(store_arr[store_idx], (0xC0 + (2)));
    }
...
void download_log(void)
{
    ...
    {
        clcd_write(0x01, 0);
        clcd_print("Downloading log", (0x80 + (0)));
...
void menu_display(void)
{
    ...
    clcd_print(menu[disp_count], (0x80 + (1)));
    clcd_print(menu[disp_count + 1], (0xC0 + (1)));


    if (key == 2 && disp_count <= 2)
    {
        if (disp_flag == 0)
        {
            disp_flag = 1;
...
void store_event(void)
{
    write_external_eeprom(addre++, time[0]);
